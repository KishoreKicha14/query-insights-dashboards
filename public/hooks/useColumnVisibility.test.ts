...
describe('useColumnVisibility', () => {
  ...
  describe('localStorage persistence', () => {
    ...
    it('keeps a version-gated column visible when it is absent on first render then appears', () => {
      // Stored prefs (new format) where 'status' is a known, visible column. Simulates a
      // version-gated column: absent from the columns array on the first render, present after
      // the async version probe resolves.
      seedStorage(
        ['type', 'timestamp', 'latency', 'cpu', 'status'],
        ['type', 'timestamp', 'latency', 'cpu', 'status']
      );

      const withoutGated = baseColumns; // no 'status' yet (gate off)
      ...
      // First render: the gate is off so the column isn't present; it isn't rendered, but its
      // preference must not be lost.
      expect(result.current.isColumnVisible('status')).toBe(false);

      // Gate flips on: the column appears and must be visible (its stored preference), not
      // dropped or treated as a brand-new column.
      rerender({ cols: withGated });
      expect(result.current.isColumnVisible('status')).toBe(true);

      // The preference must survive a reload: primary key still lists 'status' as visible.
      const storedVisible = JSON.parse(localStorage.getItem(STORAGE_KEY)!);
      expect(storedVisible).toContain('status');
    });
